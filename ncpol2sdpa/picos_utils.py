    ...
    if row_vector.getnnz()>0:
        ...
        for k in columns:
            if k==0:
                ...
                row0 = F_struct[:
                        row_offsets[block_of_last_moment_matrix+1], k].nonzero()[0][0]
...
def objective_to_affine_expression(objective, F_struct, row_offsets,
    ...
    for k, v in enumerate(objective):
        if v!=0:
            row0 = F_struct[0:
                    row_offsets[block_of_last_moment_matrix+1], k+1].nonzero()[0][0]
            block, i, j = convert_row_to_sdpa_index(block_struct,
                                                            row_offsets, row0)
...
def convert_to_picos(sdp_problem):
    ...
    for block, block_size in enumerate(sdp_problem.block_struct):
        if block>0 and block_size<sdp_problem.block_struct[block]:
    ...
    for block in range(block_of_last_moment_matrix+1):
        block_size = sdp_problem.block_struct[block]
        ...
        if sdp_problem.F_struct[start:end, 0].getnnz() > 0:
            P.add_constraint(X[block][0, 0] == 1)
        # Here we define the internal symmetries of the moment matrix
        for k in range(1, sdp_problem.n_vars+1):
            if sdp_problem.F_struct[start:end, k].getnnz() > 1:
                row0 = sdp_problem.F_struct[start:end, k].nonzero()[0][0]
                block_index, i1, j1 = \
                    convert_row_to_sdpa_index(
                        sdp_problem.block_struct,
                        row_offsets,
                        row0)
                for row in sdp_problem.F_struct[start:end, k].nonzero()[0][1:]:
                    block_index, i2, j2 = \
                        convert_row_to_sdpa_index(
                            sdp_problem.block_struct,
                            row_offsets,
    ...
    for block in range(block_of_last_moment_matrix,
        ...
        for row in range(start, end):
            row_vector = sdp_problem.F_struct.getrow(row)
            affine_expression = \
              row_to_affine_expression(row_vector, sdp_problem.F_struct,
                                       row_offsets,
                                       block_of_last_moment_matrix,
                                       sdp_problem.block_struct, X)
            if affine_expression != None:
                i, j = divmod(row-start, block_size)
                P.add_constraint(Y[i, j] == affine_expression)
    affine_expression = \
      objective_to_affine_expression(sdp_problem.obj_facvar,
                                     sdp_problem.F_struct, row_offsets,
                                     block_of_last_moment_matrix,
                                     sdp_problem.block_struct, X)
