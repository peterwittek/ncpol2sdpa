...
class SdpRelaxation:
    ...
    def __process_inequalities(self, inequalities, monomials, block_index, order):
        ...
        for ineq in inequalities:
             ...
             block_index+=1
             for i in xrange(ineq_n_monomials):
                for j in xrange(i, ineq_n_monomials):
    ...
    def get_relaxation(self, obj, inequalities, equalities, 
        ...
        for monomial in self.monomial_substitutions.keys():
            if monomials.__contains__(monomial):
                monomials.remove(monomial)
    
        self.n_monomials = len(monomials)
        self.n_vars = self.n_monomials*(self.n_monomials+1)/2
        ...
        print 'Transforming', len(equalities), 'equalities to', 2*len(equalities), 'inequalities...'
